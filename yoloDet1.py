...
class YoloTRT():
    ...
    # Filter detections to keep only vehicle class (e.g., "car")
        vehicle_det_res = []
        for j in range(len(result_boxes)):
            class_label = self.categories[int(result_classid[j])]
            if class_label in ["car", "truck", "bus", "motorcycle", "bicycle"]:
                box = result_boxes[j]
                det = dict()
                det["class"] = class_label
                det["conf"] = result_scores[j]
                det["box"] = box
                vehicle_det_res.append(det)
                self.PlotBbox(box, img, label="{}:{:.2f}".format(class_label, result_scores[j]),)

        num_vehicles_detected = len(vehicle_det_res)

        return vehicle_det_res, t2 - t1, num_vehicles_detected
